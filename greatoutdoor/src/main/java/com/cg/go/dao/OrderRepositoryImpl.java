...
public class OrderRepositoryImpl implements IOrderRepository{
	...
	public List<OrderEntity> findOrdersByUserId(String userId){
		...
		list=entityManager.createQuery("from OrderItemEntity where userId=:userId",OrderEntity.class).setParameter("userId", userId).getResultList();
		return list;
	}
	public List<OrderEntity> findAllOrders(){
		List<OrderEntity> list=new ArrayList<OrderEntity>();
		list=entityManager.createQuery("select order from OrderEntity order",OrderEntity.class).getResultList();
		return list;
	}
